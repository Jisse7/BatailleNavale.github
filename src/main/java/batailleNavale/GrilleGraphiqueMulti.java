...
class JButtonCoordonneeMulti extends JButton {

	private Coordonnee c;
	public JButtonCoordonneeMulti(Coordonnee c) {
		this.c=c;
	}
 ...
 *
 */
public class GrilleGraphiqueMulti extends JPanel implements ActionListener {
	 ...
	 */
	public GrilleGraphiqueMulti(int taille) {
		try {
		...
		for (int i = 0; i < taille; i++) {
			...
			for (int j = 0; j < taille; j++) {
				...
				cases[i][j].setBackground(BatailleNavale.gridColor);
				this.add(cases[i][j]);
